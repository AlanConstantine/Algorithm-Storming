...
class Solution:
    def maxProfit(self, prices) -> int:
        ...
        dp[0][1] = -1*prices[0]
        for i in range(1, size):
            # 第i天手里没有股票有两种可能：1:前一天也没有持有即dp[i-1][0]，或者前一天持有，但是第i天的时候卖出了则利润为dp[i-1][1]+prices[i]
            dp[i][0] = max(dp[i-1][0], dp[i-1][1]+prices[i])
            # 第i天手里持有股票也有两种可能：1:前一天没有持有股票但是今天买入了，或者前一天本身就持有股票
            dp[i][1] = max(dp[i-1][0]-prices[i], dp[i-1][1])
        return dp[size - 1][0]  # 不持有股票状态所得金钱一定比持有股票状态得到的多
